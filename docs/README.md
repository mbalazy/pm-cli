...
- [executor.md](executor.md) - the unattended modes: first the manual for `pm work`, `pm run-epic` and `pm finish`, then their internals - worktree slots, the review policy and the hook that enforces it, run state and journal, kill and crash forensics, acceptance claims, the verification baseline.
...
- [design-log.md](design-log.md) - dated decisions with the measurements behind them: what was tried, what it cost, why the current shape won.
