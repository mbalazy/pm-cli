...
`)
	...
	sb.WriteString(`- status: "verified" = implemented + reviewed-clean + verify-green (green = zero NEW failures; pre-existing breakage noted as PRE-EXISTING does not demote this); "blocked" = escalated to a human (review could not converge, or ambiguous/out-of-scope); "failed" = verify could not pass due to failures your changes introduced. Note it is "verified", not "merged": you never merge anything - what happens to your branch afterwards is the manager's call.
