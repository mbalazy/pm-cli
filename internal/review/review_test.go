...
func TestStartRunsTheReviewInTheCheckout(t *testing.T) {
	c, checkout, argvFile := setup(t)
	r, err := c.Start("https://github.com/org/app/pull/7/changes")
	if err != nil {
		t.Fatal(err)
	}
	if r.Project != "app" || r.State != StateRunning || r.URL != "https://github.com/org/app/pull/7" {
	...
	wantDir, _ := filepath.EvalSymlinks(checkout)
	if lines[0] != wantDir || lines[1] != "/tmp/cfg-company" {
		t.Errorf("cwd/config dir = %q %q", lines[0], lines[1])
	}
	argv := strings.Join(lines[2:], "\n")
	if !strings.Contains(argv, "-p\n/review https://github.com/org/app/pull/7\n") || !strings.Contains(argv, "Bash(gh pr diff:*)") {
...
func TestMatchByGitRemote(t *testing.T) {
	...
	if _, err := c.Start("https://github.com/nobody/nothing/pull/1"); err == nil || !strings.Contains(err.Error(), "no pm project checks out nobody/nothing") {
		t.Errorf("unknown repo err = %v", err)
	}
	if _, err := c.Start("not a url"); err == nil {
...
func TestFailedAndCancelledReviews(t *testing.T) {
	...
	r, err := c.Start("https://github.com/org/app/pull/8")
	if err != nil {
		t.Fatal(err)
	}
