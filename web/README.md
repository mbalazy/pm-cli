...
**What it does.** Project sidebar, tasks by status, task detail (markdown, Spec/Log zones), Runs table with an explicit remote fetch, ⌘K palette, `j`/`k`/`Enter`/`Esc`/`?` shortcuts, live refetch from `/api/events`, a phone layout (checked at 390 px; the sidebar returns as a column from 768 px, a second attention column from 820 px) and a PWA manifest. It also mutates pm data. Most writes go through `api/mutations.ts` and one confirmation dialog (status and blocker reason, brief, focus, notes, run control, solo launches); dismissing an attention row, saving settings, approving a PR and the change feed's refresh/seen post directly (the feed's two live in `api/queries.ts`).
